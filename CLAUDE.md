...
боевой AI (`04_AI.md`), сборка/редактор/конвенции (`05_WORKFLOW.md`), учебное
задание (`06_COURSE_HOMEWORK.md`).
**Перед любой задачей** сверяйся с `01_GDD.md` (какие правила); после выполнения —
синхронизируй доки. Отклонение от GDD сначала фиксируется в GDD.

**Рабочие файлы вне сдачи — в `dev/`** (в документацию проекта не входят):
план подготовки к сдаче (`RELEASE_PLAN.md`), рабочий беклог (`BACKLOG.md`),
матрица ручной приёмки (`ACCEPTANCE.md`), промты генерации контента
(`CONTENT_PROMPTS.md`), справочник по тулингу редактора
(`agents/AGENT_UNREAL_TOOLING.md`), отчёты аудитов (`audit/`).
...
- `Characters/`, `UI/`, `PCG/` — **перенесено из донора** (иерархия юнита на
  GAS, HUD-виджеты, PCG-ноды). API-макрос переименован
...
- `Hub/`, `UI/Menus/` — **написано с нуля**: голо-карта хаба и каркас экранов меню
  на CommonUI.
- `Audio/`, `FX/`, `Subtitles/` — **написано с нуля**: звуковой, визуальный и
  субтитровый слои, все ведомы Data Asset'ами и вызываются из C++ на подтверждённом
  факте, а не из BP (`docs/03_ARCHITECTURE.md` §12–§14).
- `Tactics/Editor/`, `UI/Editor/` — editor-only библиотеки авторинга (сборка графов,
  вёрстки, локализация текста ассетов); `Tactics/Tests/` — автотесты скоринга AI.
- Классы шаблона Epic (`Variant_*`, `XRU1Character/GameMode/PlayerController`) и
  донорский `Interaction/` удалены — тактический режим ими не пользовался.
...
> [dev/agents/AGENT_UNREAL_TOOLING.md](dev/agents/AGENT_UNREAL_TOOLING.md)** —
