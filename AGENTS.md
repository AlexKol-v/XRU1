...
`.mcp.json` в корне, общий для обеих машин). Новая сессия Codex подхватит
...
- **codebase-memory MCP** — семантический граф кодовой базы (`search_code`, `trace_path`,
  ...
  - **Модель установки (важно):** сервер регистрируется **ГЛОБАЛЬНО, один на все проекты** —
    в `~/.Codex.json` (ключ `mcpServers.codebase-memory-mcp` → `~/.local/bin/codebase-memory-mcp.exe`).
  ...
  - **Проверка:** `/mcp` в Codex должен показывать `codebase-memory-mcp`. Индекс
...
- **Разрешения** (`.Codex/settings.json`): allowlist на build/copy/MCP; запись в донора
  запрещена (`deny`).
