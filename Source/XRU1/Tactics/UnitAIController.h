...
// FAIContact/EAIContactSource нужны по значению: личная память контактов
// хранится массивом прямо на контроллере. Заголовок директора тянет только
// CoreMinimal и WorldSubsystem, циклической зависимости не создаёт.
...
public:
	 ...
	 *
	 * Судить занятость по velocity нельзя: после финиша боец ещё ~0.3 с гасит
	 * скорость торможением, всё это время считается «бегущим» и НЕ ставит диск —
	 * зона следующего бойца показывает его клетку свободной.
	 */
	UFUNCTION(BlueprintPure, Category = "Tactics|AI")
	bool IsMoving() const;

	/**
	 * Юнит РЕАЛЬНО перемещается по маршруту (без «занят доводкой позы»).
	 *
	 * ⚠️ Отличие от `IsMoving` принципиальное и родилось из бага: `IsMoving`
	 * намеренно включает settlement (подшаг к стене и доворот на месте), потому
	 * что для очереди приказов и зоны хода это тоже «переходное состояние». Но
	 * АНИМАЦИИ по нему верить нельзя: доворот на месте выставлял позу Moving,
	 * то есть боец вставал из укрытия в локомоцию, тут же садился обратно и
	 * только потом стрелял («встал, резко сел, потом выстрел»).
	 * Поза обязана следовать за фактическим перемещением.
	 */
	UFUNCTION(BlueprintPure, Category = "Tactics|AI")
	 ...
	 * игрока (`SquadVisionRange`) = 2500. С радиусом заметно меньше игрок
	 * спокойно расстреливает врага с дистанции, на которой тот физически не может
	 * его увидеть, — и враг честно стоит на посту, ничего не зная. Радиус
	...
	/**
	 * ПОВОДОК ЧАСОВОГО: как далеко от своего поста боец готов уйти проверять шум.
	 * 0 — без поводка: идёт к любой точке интереса.
	 *
	 * Действует ТОЛЬКО на часового — бойца без маршрута патруля и без радиуса
	 * обхода (`SentryCount` энкаунтера, «сапёр-сторож у заряда»). У патрульного
	 * поста нет, привязывать его не к чему.
	 *
	 * И действует ТОЛЬКО до вскрытия пода. Часовой охраняет объект, пока идёт
	 * перестрелка где-то ещё; как только его группу вскрыли, он воюет как все —
	 * иначе охрана заряда превратилась бы в бессмертных статистов, которых можно
	 * расстреливать по одному.
	 *
	 * Зачем: без поводка под охраны заряда уходит к шуму за десятки метров
	 * от бомбы, и объект остаётся без
	 * охраны. В XCOM 2 у групп, привязанных к цели миссии, ровно такая привязка:
	 * они не гоняются за звуками через всю карту.
	 * 1600 = ход бегом (2 ОД × MoveRange).
	 */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Tactics|Combat", meta = (ClampMin = "0"))
	float PostLeashRadius = 1600.f;

	/**
	 * Шанс встать в НАБЛЮДЕНИЕ, дойдя до последней известной точки врага и никого
	 * там не найдя (0..1). 0 — выключить: «сразу в патруль».
	 *
	...
	/**
	 * Во сколько раз быстрее ходит боец, которого игрок НЕ ВИДИТ.
	 *
	 * Ход противника длинный не из-за «думанья» AI, а из-за проигрывания
	 * перемещений — и большая их часть происходит там, куда игрок не смотрит.
	 * XCOM 2 решает это радикально: невидимые куски пути AI не визуализируются
	 * вовсе, а полностью скрытый юнит телепортируется (`X2Action_Move`).
	 * Телепорт мы сознательно не переносим — перемещение у нас механика, на
	 * которой висят реакция Overwatch и стимулы перцепции. Ускорение даёт тот же
	 * выигрыш времени, не трогая правила: 1 — выключено.
	 */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Tactics|Combat", meta = (ClampMin = "1", ClampMax = "12"))
	float HiddenMovementSpeedMultiplier = 5.f;
	...
	/**
	 * Множитель НОРМИРОВАННОЙ ценности укрытия точки.
	 *
	 ...
	 *
	 * ⚠️ Вес умножается НЕ на очки защиты (20/40), а на нормированную оценку
	 * укрытия с диапазоном входа −4…+1.1 — поэтому дефолт такого порядка.
	 */
	...
	/** Бонус точке, из которой юнит ФЛАНКИРУЕТ хотя бы одну угрозу. */
	UPROPERTY(EditDefaultsOnly, Category = "Tactics|AI|Weights")
	 ...
	 * 96 юнитов Unreal, то есть ≈576 см. Меньшие значения (вроде 250) штраф за
	 * кучу дают только когда бойцы стоят фактически вплотную, и «разойтись» он
	 * почти не заставляет. Сверено с `XComAI.ini`.
	 */
	UPROPERTY(EditDefaultsOnly, Category = "Tactics|AI|Weights", meta = (ClampMin = "0"))
	 ...
	 *
	 * Verbatim-значение 0.2, сверенное с ini XCOM: кучная позиция теряет 80%
	 * ценности.
	 */
	 ...
	 * 0.5–4.0, у Defensive максимум). Без этого члена у AI
	 * работает только анти-кучный штраф — то есть отряд умеет РАЗБЕГАТЬСЯ, но не
	 * умеет держать линию. В XCOM это два независимых механизма: минимальная
	...
	// --- Веса выбора ЦЕЛИ ------------------------------------------------
	//
	// Аддитивный скоринг, а не «ближайший видимый». Числа
	...
	/**
	 * ЧИСТАЯ ФУНКЦИЯ СКОРА ПОЗИЦИИ: факты + веса → число.
	 *
	 * Мир не читает и не меняет, поэтому проверяется автотестом без карты, без
	 * навмеша и без PIE (`XRU1AITests.cpp`). Всё, что требует трейсов и
	 * навигации, живёт в `FindCoverPoint` и приходит сюда уже посчитанным.
	 *
	 * `bRetreat` инвертирует два члена: видимость врагов становится наградой за
	 * РАЗРЫВ линии огня, а дистанция — наградой за удаление. `bAdvance` снимает
	 * штраф за потерю линии огня, чтобы промежуточное укрытие без выстрела не
	 * отвергалось (иначе бот наступает открытой пробежкой).
	 */
	static float ScorePositionFacts(const FAIPositionFacts& Facts,
		const FAIPositionScoringTuning& Tuning, float IdealCombatRange,
		bool bRetreat, bool bAdvance);

	/**
	 * Текущие (уже с учётом профиля и осей стиля) веса позиции одной структурой.
	 * Нужен, чтобы чистый скорер получал ровно то, чем живёт runtime, а не
	 * значения из ассета.
	 */
	FAIPositionScoringTuning MakePositionTuningSnapshot() const;
	...
	/**
	 * ШТРАФ ЗА ВОЗВРАТ на позицию, которую боец занимал в предыдущих ходах.
	 *
	 * Лечит «убегает за укрытие и на следующем ходу выбегает обратно».
	 * Причина метаний не в одном плохом весе: оценщики выигрывают
	 * по очереди. Из открытой позиции побеждает `MoveToCover` (нужна линия огня),
	 * из укрытия без цели — `AdvanceToCover` (потеря линии огня НЕ штрафуется и
	 * дистанция тянет вперёд). Каждое решение по отдельности разумно, а вместе
	 * они дают маятник, потому что у бойца нет памяти о том, где он только что
	 * был. `RelocateBias` тут не помогает — он сравнивает только с ТЕКУЩЕЙ точкой.
	 *
	 * Это не копия XCOM: там `CURR_TILE_LINGER_PENALTY` наоборот подталкивает
	 * двигаться. Нам нужен противоположный знак ровно потому, что у нас нет
	 * тайлов и кандидаты сэмплируются кольцами — «почти та же точка» встречается
	 * гораздо чаще. 0 — выключить.
	 */
	UPROPERTY(EditDefaultsOnly, Category = "Tactics|AI|Weights", meta = (ClampMin = "0"))
	float RecentPositionPenalty = 30.f;

	/** Насколько близко к прошлой позиции точка считается «возвратом» (см). */
	UPROPERTY(EditDefaultsOnly, Category = "Tactics|AI|Weights", meta = (ClampMin = "0"))
...
protected:
	...
	/**
	 * Действия шага в состоянии Patrol. Режим задаётся парой
	 * `PatrolPoints` + `PatrolRoamRadius` (таблица — у `AUnitBase::PatrolPoints`):
	 * пост, удержание точки, свободный обход зоны или маршрут по точкам.
	 */
	bool StepPatrol(AUnitBase* Unit);

	/** Боец остаётся на месте и уходит в наблюдение (пост / удержание точки). */
	bool HoldPositionOnPost(AUnitBase* Unit, const TCHAR* Reason);

	/**
	 * Часовой: боец охраняет точку, а не патрулирует. Признак — пустой маршрут
	 * и нулевой радиус обхода, ровно то, что ставит `SentryCount` энкаунтера.
	 */
	bool IsPostSentry(const AUnitBase* Unit) const;

	/**
	 * ПОСЛЕДНЯЯ СТУПЕНЬ ЛЕСТНИЦЫ ФОЛБЭКОВ мирного хода: сорвавшееся перемещение
	 * не имеет права закончить ход ничем.
	 *
	 * Порядок: наблюдение (держим сектор) → глухая оборона, если под рукой есть
	 * укрытие → честный «пропуск с причиной» со списанием ОД. Последняя ступень
	 * нужна не ради действия, а ради ЗАВЕРШИМОСТИ: без списания очка шаг хода
	 * повторил бы тот же отказ, а ход бойца остался бы висеть.
	 *
	 * Звать `HoldPositionOnPost` ТОЛЬКО из ветки «уже стою на посту» нельзя:
	 * провал маршрута патруля и провал разведки остаются без фолбэка вовсе —
	 * это и давало большинство «ходов БЕЗ ДЕЙСТВИЙ».
	 */
	bool FallbackHoldOrSkip(AUnitBase* Unit, const TCHAR* Context);

	/**
	 * Шаг свободного обхода зоны: случайная достижимая точка в радиусе вокруг
	 * якоря. `RadiusOverride > 0` перебивает `AUnitBase::PatrolRoamRadius` —
	 * это нужно, когда обход служит АВАРИЙНЫМ выходом для бойца без своего
	 * радиуса (маршрут не строится): радиус берётся от дальности хода, иначе
	 * «зона» вырождается в метр вокруг ног и ни одна точка не подходит.
	 */
	bool StepRoamAroundAnchor(AUnitBase* Unit, const FVector& Anchor, float RadiusOverride = 0.f);

	/** Переводит `PatrolIndex` на следующую точку с учётом режима маршрута. */
	void AdvancePatrolIndex(const AUnitBase* Unit);

	/**
	 * С какой точки боец входит в маршрут: ближайшая к его позиции плюс
	 * смещение группы. Считается один раз за вселение.
	 */
	int32 ResolveInitialPatrolIndex(const AUnitBase* Unit) const;

	/** Стартовая точка маршрута уже выбрана в этом вселении. */
	bool bPatrolIndexResolved = false;

	/** Центр зоны удержания (стартовая позиция бойца), если точка не задана. */
	FVector PatrolAnchorLocation = FVector::ZeroVector;
	bool bPatrolAnchorSet = false;

	/** Направление обхода для `PingPong` (+1 вперёд, −1 назад). */
	int32 PatrolDirection = 1;

	/**
	 * РАЗЛИЧНЫЕ позиции бойца за последние ходы (свежая — первая). Нужны только
	 * для `RecentPositionPenalty`: память о том, откуда он только что ушёл.
	 *
	 * ⚠️ Именно РАЗЛИЧНЫЕ, а не «позиции последних N ходов». Боец двигается не
	 * каждый ход (ход через ход он стреляет), поэтому список из позиций подряд
	 * забивался бы дублями одной точки и помнил бы всего одно место. Маятник же
	 * имеет период до трёх ходов, а точки соседних заходов расходятся на полтора
	 * метра — память до второго шага не достаёт.
	 */
	TArray<FVector> RecentTurnPositions;
	static constexpr int32 MaxRecentPositions = 3;

	/**
	 * Движение к точке с обрезкой по бюджету пути юнита. true — движение началось.
	 *
	 * `RequiredGoalTolerance > 0` означает «дойти ИМЕННО СЮДА»: приказ
	 * принимается, только если конец построенного маршрута ближе допуска к цели.
	 * Это обязательно для боевого манёвра и запрещено для патруля.
	 *
	 * Зачем разделение. У обоих планировщиков есть
	 * собственные «почти-цели»: поле дистанций при неудаче берёт БЛИЖАЙШИЙ
	 * достижимый сэмпл к цели, а прямой навмеш-путь обрезается бюджетом хода.
	 * Для патруля это ровно то, что нужно — идти в сторону маршрута. Для манёвра
	 * это подмена: `FindCoverPoint` оценил КОНКРЕТНУЮ точку (укрытие от 4 угроз,
	 * линия огня), а боец уезжал за 21 метр от неё и вставал в чистом поле —
	 * `встал НЕ в выбранную точку — расхождение 2132 см, укрытие на месте: 0`.
	 * Снаружи это и читалось как «убежал, пропал из виду и вернулся».
	 */
	bool MoveWithBudget(AUnitBase* Unit, const FVector& Goal, float AcceptanceRadius,
		int32 MaxActionPoints = 1, float RequiredGoalTolerance = 0.f);

	/** Допуск «дошёл именно туда» для боевого манёвра (см). */
	UPROPERTY(EditDefaultsOnly, Category = "Tactics|AI|Positioning", meta = (ClampMin = "10"))
	float ManeuverGoalTolerance = 150.f;

	// --- Диагностика простоя ---------------------------------------------------
	//
	// Warning «закончил ход БЕЗ ДЕЙСТВИЙ» печатал СОСТОЯНИЕ (alert, AP, число
	// точек патруля), но не ПРИЧИНУ. Из него нельзя понять даже того, чего не
	// хватило: маршрута, навмеша или способности —
	// каждый следующий разбор начинался с гадания. Причины копятся цепочкой по
	// мере спуска по лестнице фолбэков и печатаются целиком.

	/** Цепочка причин, по которым ход не превратился в действие («A → B → C»). */
	FString TurnIdleReasons;

	/** Добавляет звено в цепочку причин (не более `MaxIdleReasons`). */
	void NoteIdleReason(const FString& Reason);

	/** Сколько звеньев причины помещается в одну строку лога. */
	static constexpr int32 MaxIdleReasons = 4;

	/**
	 * Почему отказал последний `MoveWithBudget`. Заполняется внутри него самого:
	 * вызывающему негде взять «поле не построилось» или «план дороже бюджета».
	 */
	FString LastMoveFailure;

	/**
	 * Стоит ли боец на навмеше и где ближайшая подтверждённая точка.
	 *
	 * Ровно этот случай ловит `[Encounter] ... поставлен без подтверждения
	 * навмешем`: боец вне навмеша не строит НИ ОДНОГО маршрута, потому что поле
	 * дистанций (`AMoveRangeVisualizer::BuildDistanceField`) начинается с
	 * проекции его собственной позиции. В логе простоя это должно быть видно
	 * сразу, а не выясняться клавишей `P` в PIE.
	 */
	bool ProjectOntoNavmesh(const FVector& Location, FVector& OutProjected,
		const FVector& Extent = FVector(150.f, 150.f, 300.f)) const;

	/** Предупреждение «боец вне навмеша» печатается один раз на вселение. */
	bool bReportedOffNavmesh = false;

	/** Завершает ход юнита и уведомляет TurnManager. */
	void FinishUnitTurn();

	/** Планирует следующий шаг хода через ActionInterval (у скрытого — на тик). */
	void ScheduleNextStep();

	/** Видит ли отряд игрока этого бойца прямо сейчас (инверсия — «скрыт»). */
	bool IsHiddenFromSquad() const;

	/** Ставит скорость по видимости: скрытый идёт ускоренно, видимый — обычно. */
	void ApplyHiddenMovementSpeed();

	/** Возвращает исходную скорость (конец хода, потеря пешки). */
	void RestoreMovementSpeed();

	UFUNCTION()
	void HandleFogVisibilityChanged(AActor* Actor, bool bVisible);

	/** Настоящая скорость бойца, запомненная до ускорения (0 — ещё не бралась). */
...
public:
	 ...
	 * засчитывалась», оставаясь вечно открытой.
	 */
	 ...
	 * остаток ОД НЕ отдаётся utility-AI (иначе после провала постановки враг
	 * тратит остаток ОД на свободный выстрел).
	 */
...
protected:
	...
	/**
	 * Лучшая цель по АДДИТИВНОМУ СКОРИНГУ (правила XCOM — см.
	 * docs/08_AI.md): шанс попадания + провокация + фланг +
	 * добивание + ранение, минус штраф за тяжелораненого.
	 *
	 * «Ближайший видимый, провоцирующий вне очереди» здесь не годится:
	 * расстояние не коррелирует ни с шансом попасть
	...
	/** Все живые враждебные акторы, видимые перцепцией (для скоринга против ВСЕХ угроз). */
	...
	/**
	 * Рабочая точка разведки. НЕ источник правды: выводится из `ContactMemory`
	 * методом `RefreshInvestigateTarget` — так весь код, писавшийся до памяти
	 * контактов, продолжает работать, но выбор точки стал осознанным.
	 */
	FVector LastKnownThreatLocation = FVector::ZeroVector;
	bool bHasThreatLocation = false;

	// --- Личная память контактов --------------------------------------
	//
	// До этого у бойца была ОДНА точка `LastKnownThreatLocation`, и любое
	// событие её перетирало: потерял из виду A — забыл, что секунду назад
	// стрелял по B. Память пода (`UTacticalAIDirectorSubsystem`) эту дыру
	// закрывала лишь частично: она общая на группу и обновляется по её
	// правилам, а вопрос «куда идти ЭТОМУ бойцу» личный.
	//
	// ⚠️ Память даёт право ЗНАТЬ, а не право стрелять: кандидат в цели всё равно
	// проходит обычный `UGA_Attack::CanTargetActor` с реальными дальностью и
	// линией огня (§7).

	/** Что боец знает лично: несколько контактов, каждый со своим источником. */
	TArray<FAIContact> ContactMemory;

	/** Сколько ходов боец помнит неподтверждённый контакт. */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Tactics|AI|Memory", meta = (ClampMin = "1"))
	int32 ContactMemoryTurns = 3;

	/** Больше этого числа контактов не храним: старший по достоверности вытесняет. */
	static constexpr int32 MaxContactMemory = 6;

	/**
	 * Запомнить контакт. `Target == nullptr` — «подозрение» (шум): известна
	 * только точка, актора боец не видел.
	 */
	void RememberContact(AActor* Target, const FVector& Location,
		EAIContactSource Source, float Confidence);

	/** Самый достоверный живой контакт. false — боец ничего не знает. */
	bool GetBestContact(FAIContact& OutContact) const;

	/** Затухание по ходам и вычистка протухших. Зовётся в начале активации. */
	void AgeContactMemory();

	/** Пересобирает `LastKnownThreatLocation` из памяти (лучший контакт). */
	void RefreshInvestigateTarget();
	...
	/**
	 * Стоимость запланированного перемещения в AP. Финализация с жёстким
	 * списанием 1 AP заставляла бы планировать манёвр бюджетом в
	 * одно очко — и боец останавливался бы на полпути к укрытию.
	 */
