...
// FAIContact/EAIContactSource нужны по значению: личная память контактов (AI-2)
...
public:
	 ...
	 * только потом стрелял («встал, резко сел, потом выстрел» — запись PIE
	 * 2026-08-03). Поза обязана следовать за фактическим перемещением.
	 */
	UFUNCTION(BlueprintPure, Category = "Tactics|AI")
	 ...
	 * 0 — без поводка (прежнее поведение: идёт к любой точке интереса).
	 *
	 ...
	 *
	 * Зачем: в прогоне 2026-08-04 под 6 (двое у заряда) ушёл к шуму на
	 * (3317, −2767) — за шесть десятков метров от бомбы, — и объект остался без
	...
	/**
	 * ЧИСТАЯ ФУНКЦИЯ СКОРА ПОЗИЦИИ (AI-3): факты + веса → число.
	 *
	 ...
	 *
	 * Лечит «убегает за укрытие и на следующем ходу выбегает обратно» (прогон
	 * 2026-08-04). Причина метаний не в одном плохом весе: оценщики выигрывают
...
protected:
	 ...
	 *
	 * Раньше `HoldPositionOnPost` вызывался ТОЛЬКО из ветки «уже стою на посту»,
	 * поэтому провал маршрута патруля и провал разведки не имели фолбэка вовсе —
	 * это и давало 76 из 118 «ходов БЕЗ ДЕЙСТВИЙ» в прогоне 2026-08-04.
	 */
	 ...
	 * забивался бы дублями одной точки и помнил бы всего одно место. По логу
	 * 2026-08-04 маятник у `C_10` имел период три хода — 18-й и 21-й ходы вели
	 * в точки в 148 см друг от друга, а память до второго шага не доставала.
	 */
	 ...
	 *
	 * Зачем разделение (прогон 2026-08-04). У обоих планировщиков есть
	...
	// --- Диагностика простоя (бриф AI §4.6) -----------------------------------
	//
	// Warning «закончил ход БЕЗ ДЕЙСТВИЙ» печатал СОСТОЯНИЕ (alert, AP, число
	// точек патруля), но не ПРИЧИНУ. По прогону 2026-08-04 из него нельзя было
	// понять даже того, чего не хватило: маршрута, навмеша или способности —
	// каждый следующий прогон начинался с гадания. Причины копятся цепочкой по
