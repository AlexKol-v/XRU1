...
public:
	 ...
	 * лимиты и стоимость у способностей разные.
	 */
	 ...
	 *
	 * ⚠️ Единственная законная точка доворота: узел `FaceActorTowards` в
	 * BP_GA_Attack/BP_GA_Overwatch на этой роли доворачивает корпус ОДНИМ КАДРОМ
	 * между приходом на позицию и стартом стрелкового montage — отсюда
	 * «выбежал вбок, щёлкнул на цель, выстрелил». Возвращать его нельзя.
	 *
	 ...
	 *
	 * ⚠️ Узлу `Face Actor Towards` в BP на этой роли не место: он стоит
	 * ПЕРЕД montage и потому ломает единственную стойку, где доворот идёт уже
	 * во время анимации (OverCover): корпус щёлкает до подъёма, и «встал →
	 * довернулся → выстрелил» превращается в «щёлкнул → встал → выстрелил».
...
protected:
	...
	/** Останавливает стрелковый montage завершённой транзакции, если он ещё играет. */
	void StopFireMontage(const FTacticalFireActionContext& FinishedAction) const;

	/** Проверка исхода боя после действия (победа/поражение — у TurnManager). */
	void CheckCombatOutcome() const;
