...
public:
	 ...
	 *
	 * Дефолт false — обе миссии демо идут с чёрной картой.
	...
	/**
	 * Останавливать ли игру на время реплики.
	 *
	 * В ОБУЧЕНИИ — да: инструктаж ведёт игрока, и следующий шаг не должен
	 * начаться, пока «Купол» не договорил. В БОЕВОЙ МИССИИ — нет: игрок ходит и
	 * действует сразу, а реплики только комментируют происходящее. Блокировка
	 * ввода там превращает каждую фразу в микро-катсцену и в худшем случае
	 * читается как зависание.
	 */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Scenario")
	bool bBeatsBlockInput = true;

	/**
	 * Таблица реплик боя: что говорится и по какому событию.
	 *
	 * Живёт ОТДЕЛЬНО от StateTree, потому что реакции боя происходят в
	 * произвольном порядке и могут не случиться вовсе — дереву они не нужны, а
	 * состоянию из одних фоновых задач противопоказаны (разбор — docs/03_ARCHITECTURE.md §9).
	 * Пусто — сценарий молчит, это штатная настройка.
	 */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Scenario|Озвучка")
	TSoftObjectPtr<class UMissionVoiceDataAsset> VoiceLines;
