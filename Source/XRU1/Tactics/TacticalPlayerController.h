...
/**
 * ЕДИНЫЙ ИСТОЧНИК ПРАВДЫ о режиме взаимодействия игрока в свою фазу.
 * Два независимых булевых флага (bAwaitingAttackTarget/AbilityTarget)
 * с ручными условиями входа/выхода в каждой точке здесь нельзя — отсюда
 * «камера зависла» (один из путей выхода забывает её вернуть). ЛЮБОЙ переход идёт через
 ...
 * корневой UI-layout через UGameUIManagerSubsystem.
 */
UCLASS(Abstract)
...
public:
	 ...
	 *     разыгрывается, и только потом он бежит дальше. Без остановки кадр
	 *     гоняется за уезжающей спиной — отсюда «некрасиво».
...
protected:
	 ...
	 * и без этого предиката он проходил бы мимо всех запретов.
	 */
...
protected:
	 ...
	 * зрения — выбежал из-за угла, и камера подхватила его на бегу.
	 * Одноразовая проверка в `HandleEnemyUnitActivated` этот случай теряет
	 * целиком: враг весь ход остаётся за кадром, хотя игрок его уже видит.
...
protected:
	...
	/**
	 * Возврат камеры после акцента первого обнаружения. Удержание акцента
	 * истекает по таймеру камеры, но снятие лишь применяет ОТЛОЖЕННЫЙ интент —
	 * а если его нет (никто не бежит), камера просто оставалась на враге, и
	 * взгляд приходилось возвращать руками. Таймер чуть длиннее удержания;
	 * возврат уступает всему, что успело завладеть камерой (кадр выстрела,
	 * режиссура, следование, рука игрока).
	 */
	FTimerHandle FirstSightedReturnTimer;
	void ReturnCameraAfterFirstSighted();
