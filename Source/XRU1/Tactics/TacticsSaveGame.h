...
public:
	...
	/**
	 * Вводная Купола про оперативную карту уже прозвучала.
	 *
	 * Живёт в СЛОТЕ, а не в сессии: «Продолжить» — это возвращение игрока,
	 * который хаб уже видел, и объяснять ему заново, что перед ним за карта,
	 * незачем. Сессионный флаг такого различить не может — он одинаково пуст и
	 * после «Новой игры», и после «Продолжить».
	 */
	UPROPERTY(BlueprintReadWrite, SaveGame, Category = "Tactics|Save")
	bool bHubBriefed = false;

	/**
	 * LEGACY, не использовать. Настройки живут в `UTacticsUserSettings`:
	 * громкость и качество — параметры приложения, а не прогресса,
