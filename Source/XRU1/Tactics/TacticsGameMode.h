...
protected:
	...
	/**
	 * Поднимает директор реплик по таблице активного сценария.
	 *
	 * Отдельно от старта боя намеренно: `Scenario.Ready` приходит раньше, чем
	 * бой (он ждёт готовности навмеша), и подписчик обязан существовать к этому
	 * моменту. Идемпотентен.
	 */
	void StartMissionVoiceDirector();
