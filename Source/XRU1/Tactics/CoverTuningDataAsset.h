...
public:
	...
	/**
	 * ПОЛУУГОЛ ЗАЩИТНОЙ ДУГИ одной стены (град). 90 = стена прикрывает ровно
	 * полуплоскость со своей стороны, атака из другой половины — фланг. Это
	 * модель XCOM и то, чего просил игрок («если мы в угле 180 от укрытия — это
	 * фланг»).
	 *
	 * До S1 «укрытие против стрелка» решалось ОДНИМ лучом «есть ли стена ровно в
	 * сторону стрелка». На захламлённой карте луч почти всегда во что-то
	 * попадал, и жёлтый щит появлялся лишь в узком просвете между ящиками.
	 * Теперь стороны собираются по НОРМАЛЯМ стен и склеиваются, а дуга задаёт
	 * защиту явно.
	 *
	 * ⚠️ **Почему не 90, хотя «полуплоскость» — это ровно 90.** При 90 стена,
	 * стоящая СТРОГО ПЕРПЕНДИКУЛЯРНО линии огня, попадает на границу дуги и
	 * засчитывается как укрытие. Визуально это «боец стоит сбоку от ящика», а
	 * игра говорит «он прикрыт» — ровно та жалоба, что боты «встают не за
	 * укрытие». 70° даёт дугу 140°: перпендикулярная стена уже не спасает, а
	 * угол ящика (две стены) по-прежнему прикрывает 280°.
	 *
	 * Меньше → фланговать легче. Больше 90 не имеет смысла: стена не прикрывает
	 * сзади.
	 */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Cover|Detect", meta = (ClampMin = "30", ClampMax = "90"))
	float CoverArcHalfAngle = 70.f;
	 ...
	 * без этого на карте с плотной застройкой набиралось 3–4 «стороны» из
	 * разных ящиков и фланг становился невозможен.
	 */
