...
struct FUnitVisualState
{
	...
	/**
	 * СТОРОНА УКРЫТИЯ в осях юнита: −1 — стена слева, +1 — справа, 0 — юнит не
	 * стоит вдоль стены (значит и сторону выбрать нельзя). Именно по ней ABP
	 * берёт `Left`/`Right`-вариант cover-клипа.
	 *
	 * ⚠️ Значение осмысленно ровно потому, что `AUnitBase::HugCover()` ставит
	 * бойца ВДОЛЬ стены лицом к краю укрытия: тогда стена гарантированно сбоку.
	 * При развороте носом в стену Y вырождается в шум, и сторона становится
	 * монеткой — с этого и начинались «вывернутые» позы.
	 */
	UPROPERTY(BlueprintReadOnly, Category = "Tactics|Visual")
