...
public:
	 ...
	 * только AI (скоринг позиции); на выстрел не влияет.
	 *
	...
	/**
	 * Маршрут патруля этого юнита (TargetPoint'ы уровня). Массив читается как
	 * приказ, и вместе с `PatrolRoamRadius` даёт четыре разных поведения:
	 *
	 *  | точек | радиус | поведение                                        |
	 *  |-------|--------|--------------------------------------------------|
	 *  | 0     | 0      | пост: стоит и держит сектор наблюдением          |
	 *  | 0     | > 0    | бродит вокруг СВОЕЙ стартовой позиции            |
	 *  | 1     | 0      | удержание: дойти до точки и остаться наблюдать   |
	 *  | 1     | > 0    | бродит вокруг ЭТОЙ точки                         |
	 *  | 2+    | —      | циклический обход в порядке массива              |
	 */
	UPROPERTY(EditInstanceOnly, BlueprintReadOnly, Category = "Tactics|AI")
	TArray<TObjectPtr<AActor>> PatrolPoints;

	/**
	 * Радиус свободного блуждания вокруг точки удержания (см). 0 — не бродить.
	 *
	 * Аналог `BaseRoamRange` XCOM 2: неактивная группа там не стоит столбом и не
	 * марширует по рельсам, а перемещается в пределах своей зоны. Точка каждый
	 * ход выбирается заново, но розыгрыш детерминирован (общее правило AI: одно
	 * и то же состояние даёт одно и то же решение, иначе лог решений врёт).
	 */
	UPROPERTY(EditInstanceOnly, BlueprintReadOnly, Category = "Tactics|AI", meta = (ClampMin = "0"))
	float PatrolRoamRadius = 0.f;

	/**
	 * Как обходить маршрут из двух и более точек. Для линии точек (вдоль дороги,
	 * стены, коридора) нужен `PingPong`: иначе после последней точки боец бежит
	 * к первой через весь маршрут вхолостую.
	 */
	UPROPERTY(EditInstanceOnly, BlueprintReadOnly, Category = "Tactics|AI")
	EPatrolRouteMode PatrolRouteMode = EPatrolRouteMode::Loop;

	/**
	 * Входить в маршрут с БЛИЖАЙШЕЙ точки, а не с первой в массиве.
	 *
	 * Группу почти всегда ставят там, где она должна стоять, — в середине
	 * маршрута, у своего поста, на своём участке дороги. Без этого правила
	 * боец на первом же ходу разворачивается и бежит к точке №1 через полкарты:
	 * расстановка ломается ровно в тот момент, когда игрок её впервые видит.
	 */
	UPROPERTY(EditInstanceOnly, BlueprintReadOnly, Category = "Tactics|AI")
	bool bPatrolStartFromNearest = true;

	/**
	 * Смещение стартовой точки внутри маршрута. Нужно, чтобы группа с ОДНИМ
	 * общим маршрутом растягивалась по нему, а не шла колонной в одну вершину.
	 * Складывается с ближайшей точкой, если включён `bPatrolStartFromNearest`.
	 *
	 * Порядок точек при этом у всех одинаковый — иначе `PingPong` разворачивался
	 * бы не там, где нарисовал дизайнер.
	 */
	UPROPERTY(EditInstanceOnly, BlueprintReadOnly, Category = "Tactics|AI", meta = (ClampMin = "0"))
	int32 PatrolStartIndex = 0;
	 ...
	 * камера ещё держала кадр («садится мгновенно после выстрела»). Флаг
	 * оставляет позу стоя до конца удержания кадра, и посадка совпадает с
	 * уходом камеры.
	 *
	 ...
	 * cover, достаточно близка для прижатия). Фиксированный лимит 45 см
...
public:
	// --- Монтажи действий (дизайнерские слоты) --------------------------------
	//
