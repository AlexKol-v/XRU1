...
struct FAICoverPointResult
{
	...
	/**
	 * Сколько угроз В НАБЛЮДЕНИИ простреливают эту точку (`SafeToMove`).
...
/**
 * ИЗМЕРЕНИЯ ОДНОЙ ТОЧКИ — вход чистой функции скоринга.
 *
...
struct FAIPositionFacts
{
	...
	/**
	 * Сколько угроз в наблюдении простреливают точки ПО ПУТИ.
...
struct FAIDecisionContext
{
	...
	/** Все живые видимые враги — нужны для скоринга укрытия против ВСЕХ. */
	UPROPERTY()
	...
	/**
	 * ЛИМИТ АТАКУЮЩИХ достигнут (XCOM `MaxEngagedEnemies`): стрелять этому
