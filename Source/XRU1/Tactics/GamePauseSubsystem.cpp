...
void UGamePauseSubsystem::SetWindowFocused(bool bFocused)
{
	...
	{
		// Старт PIE: окно игры создаётся ненадолго БЕЗ фокуса, и реакция «сразу
		// пауза» вешала бы её на только что запущенный уровень — звук глушится,
		// ввод хаба блокируется, карта «не крутится». Пауза по фокусу имеет смысл
...
void UGamePauseSubsystem::ApplyPauseState()
{
	...
	// Режим ввода подсистема НЕ трогает намеренно: ставить отсюда
	// UIOnly/GameAndUI — делать хуже, это возвращает главному меню игровой режим
	// и отбирает управление у хаба, если причина паузы почему-то залипла.
