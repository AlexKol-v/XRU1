...
public:
	...
	/**
	 * ПОЛНЫЕ ПРЕДЛОЖЕНИЯ (AI-4): 0..N вариантов вместо одного.
	 *
	 * Зачем понадобилось. `PrimaryThreat` выбирается ДО сравнения действий, то
	 * есть цель фиксируется раньше, чем становится понятно, что вообще делать.
	 * Это локальный оптимум: лучшая цель для выстрела не обязана быть лучшей
	 * целью для сближения или фланга, а вариант «выстрелить в другого» до этой
	 * правки не рассматривался вовсе.
	 *
	 * База по умолчанию сводит метод к одному `ScoreAction` — оценщики, которым
	 * перебор целей не нужен, не переписываются вообще.
	 *
	 * ⚠️ ГРАНИЦА ПЕРЕБОРА. Разворачивать по всем целям имеет смысл только
	 * ДЕШЁВЫМ вариантам (выстрел — это шанс попадания, то есть арифметика).
	 * Позиционные оценщики зовут `FindCoverPoint` — 48 точек с трейсами на
	 * каждый вызов; умножить это на число целей значит утроить-учетверить время
	 * хода противника ради выбора, который почти всегда совпадает с
	 * `PrimaryThreat`. Поэтому они остаются одним предложением, и это записано
	 * как осознанное ограничение в docs/08_AI.md §4/AI-4.
	 */
	virtual void ProposeActions(const FAIDecisionContext& Context,
		TArray<FAIDecision>& OutProposals) const
	{
		FAIDecision Candidate;
		const float RawScore = ScoreAction(Context, Candidate);
		if (RawScore > 0.f)
		{
			Candidate.Score = RawScore;
			OutProposals.Add(MoveTemp(Candidate));
		}
	}
