...
public:
	...
	// 0.75 этого угла (`X2Camera_Midpoint.FramingFOVPercentage`). 90°
	...
	//
	// На максимальном приближении постоянная скорость 2000 см/с проносит экран
	...
	//
	// МОДЕЛЬ. Камера строится НАПРЯМУЮ в мировых
	...
	// результат не могут разойтись. Вынос плеча задан в СМ, а не углом
	// (в упор угловое разведение фигур вырождается), и позиция выбирается
	// лестницей кандидатов (плечо × подъём) — берётся та, откуда читаются
	// обе фигуры, а не только «камера не за стеной».
	 ...
	 *
	 * ⚠️ Числа втрое меньше «сверху сзади»: в XCOM ракурс
	 * буквально в 20–30 см позади плеча. Большой вынос (сотни см) — это не
	 * «из-за плеча»: стрелок в кадре превращается в точку. Настоящий OTS: спина
	 ...
	 * просто не видит («снайпер стреляет странно»: на 35 м цель закрыта
	 * штрафом целиком). Дальний кадр ставит камеру
	 ...
	 * съёмки ЛИПКАЯ между перекадровками и меняется, только если с
...
protected:
	 ...
	 * камерой), но без него его просто не видно в кадре — см. EnterShotFraming.
	 */
