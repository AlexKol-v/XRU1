...
public:
	...
	// --- Тюнинг укрытий/LOS/высоты (Data Asset) --------------------------------
	//
	// Все числовые параметры укрытий, линии видимости, выглядывания и высоты
	// живут в UCoverTuningDataAsset — им нельзя быть размазанными по static
	// constexpr здесь, UPROPERTY на UCoverDetectionComponent и литералу множителя
	// hunker в ComputeHitChance. Единый доступ — через GetCoverTuning.

	/**
	 * Ассет тюнинга укрытий: GameInstance->CoverTuning, иначе CDO (дефолты
	 * равны поведению без ассета — назначение ничего не меняет). НИКОГДА не
	 ...
	 * до −15 на дальней. Через него дистанция ВЛИЯЕТ на выстрел — без него 30 см
	 * и 3000 см дают одинаковые 75%.
	 */
	UFUNCTION(BlueprintPure, Category = "Tactics|Combat")
	...
	// UCoverTuningDataAsset, читаются через GetCoverTuning.
	 ...
	 *
	 * Два пути. БЫСТРЫЙ: центр + грубый step-out
	 ...
	 * nullptr — вызовы без стрелка (AI-план) остаются на быстром пути.
	 */
	 ...
	 * «AI видит цель по пику, а замороженная позиция её не видит».
	 */
	 ...
	 * для стрелка и для цели (симметрия Ф5) — отдельной функции для цели
	 * заводить не нужно.
	 *
	 ...
	 * анимации и превью). Тонкая обёртка над `FindFiringSolution` для
	...
	/**
	 * ФЛАНГ: цель СТОИТ в укрытии «вообще» (BestCoverAround != None),
	 ...
	 * Ничего не кэшируем: трейсы считаются на месте, иначе
	...
	/**
	 * ЕДИНЫЙ источник состояния щита цели против стрелка. Все UI-поверхности
	 ...
	 * `FindCoverPoint`. Усечение по клиренсу здесь делало бы недостижимой любую
	 ...
	 * мимо» — СОЗНАТЕЛЬНО. Разные правила (встать — 60, пройти —
	 * 60+радиус) дают неразрешимое противоречие: клетка, куда встать
	...
	/**
	 * Точка занята диском юнита (в 2D, квадрат просвета передаётся снаружи).
	 * Единственная реализация предиката занятости — им судят и огневые позиции,
	 * и кандидаты AI, и поле достижимости.
	 */
	static bool IsPointBlockedByUnits(const TArray<FVector>& Obstacles, const FVector& Point, double ClearanceSq);
