...
bool ABombObjective::TryDefuse(AUnitBase* Unit)
{
	...
	if (bComplete)
	{
		...
		// а на месте продолжали гореть его визуальные маркеры
		// («круг разминирования так и остаётся гореть»).
		//
...
bool AEvacZone::IsUnitInside(const AUnitBase* Unit) const
{
	...
	// Прямоугольный режим: вход считается по локальному боксу — ровно
