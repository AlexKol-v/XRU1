 ...
 *
 * `LogTemp` тактическому коду запрещён: с ним решение AI, событие квеста,
 * старт сценария и ошибка звука лежат
...
/**
 * Туман войны: кто и почему стал видим/скрыт, чем вызван пересчёт.
 *
 * Отдельная категория по той же причине, что у камеры: «врага не видно» и
 * «врага видно, хотя не должно» — это баги без места. По журналу обязано
 * восстанавливаться, какое СОБЫТИЕ вызвало пересчёт, какой ИСТОЧНИК зрения
 * дал видимость и какое ПРАВИЛО (авто / сценарный форс / труп) победило.
 */
XRU1_API DECLARE_LOG_CATEGORY_EXTERN(LogXRU1Fog, Log, All);
