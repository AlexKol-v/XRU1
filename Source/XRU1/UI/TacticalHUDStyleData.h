...
public:
	 ...
	 * `UTutorialStyleData` и может быть другим материалом. Из одного поля темы
	 * их брать нельзя: боевая подсказка и обучение оказываются склеены.
	 */
	...
	/**
	 * Щит «флангирован». Можно НЕ ЗАПОЛНЯТЬ: без своей текстуры берётся
	...
	//
	// ⚠️ Здесь осталась ОДНА карточка портрета: панели HUD собраны в Designer,
	// экраны меню верстает `UXRU1WidgetAuthoringLibrary`. Настройка, которую
	// никто не применяет, хуже её отсутствия: дизайнер крутит значение и не
	...
	/**
	 * Текстура щита по состоянию против стрелка. ShieldCover — форма щита
