...
public:
	...
	/**
	 * ПРОГРЕВ эффектов профиля перед боем.
	 *
	 * ⚠️ Зачем. В редакторе Niagara компилирует скрипты системы ЛЕНИВО — к
	 * первому спавну. Первый выстрел в PIE поэтому стоил фриза: в логе прямо в
	 * кадре выстрела шли три компиляции подряд (`NS_MuzzleFlash` 0.17 с,
	 * `NS_BulletTracer` 0.05 с, `NS_Impact_Concrete` 0.19 с — запись 2026-08-03),
	 * а все следующие выстрелы были гладкими. Прогрев переносит эту цену на
	 * старт боя, где кадр и так не показывают.
	 *
	 * В упакованной игре скрипты уже скомпилированы, поэтому тело метода —
	 * редакторное; вызов остаётся безусловным и в билде ничего не стоит.
	 */
	void WarmUpEffects();
