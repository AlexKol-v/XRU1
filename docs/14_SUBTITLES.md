...
3. ~~**Экран настроек.**~~ **СДЕЛАНО 2026-08-03.** В `WBP_Settings` есть секция
   «СУБТИТРЫ И ЯЗЫК» с `Chk_Subtitles`, `Chk_SubtitleSpeakers`,
   `Cmb_SubtitleSize`, `Cmb_SubtitleBackdrop`, `Cmb_Language`. Вёрстка — в
   `UXRU1WidgetAuthoringLibrary::BuildSettingsMenuLayout`, а не руками в
   Designer: экран собирается программно целиком, и ручная правка потерялась бы
   при следующей пересборке. Проверено в PIE: C++ привязался и заполнил списки
   (3 размера, 3 подложки, 2 языка), текущие значения читаются.
   Опции в Designer не заводить — их порядок задаёт `NativeOnInitialized`.
...
### 8.3. Владение репликой: экран против мира

Реплика, начатая ЭКРАНОМ (брифинг, итог операции), обязана умереть вместе с ним:
`UMenuScreenBase::PlayScreenVoice()` запоминает звуковой компонент, а
`NativeDestruct` его останавливает. Субтитр снимать отдельно не нужно — он
привязан к тому же компоненту, и `Stop()` шлёт `OnAudioFinished`.

Точка снятия именно `NativeDestruct`, а не `NativeOnDeactivated`: деактивация
означает лишь «экран ушёл под другой» (настройки поверх брифинга), и рвать там
реплику нельзя — её и так ставит на паузу `UGamePauseSubsystem` вместе с миром.
Это то же правило, по которому экран отпускает паузу и фон.

Реплики МИРА (вводная хаба, реплики боя) владельца-экрана не имеют и живут
своей длительностью — их обрывает только следующая реплика.

### 8.4. Осталось в коде мелочью
...
1. **Смена уровня.** `PostLoadMapWithWorld` приходит ПОЗЖЕ, чем `BeginPlay`
   акторов нового мира, а реплики оттуда уже идут (вводная хаба — из
   `AHubGameMode::BeginPlay`). Поэтому уборка на этом событии решается по
   принадлежности миру: строка и оверлей ПРОШЛОГО мира снимаются, строка нового
   остаётся и получает оверлей заново. Слепая уборка гасила вводную хаба ровно
   в момент её показа — субтитр не появлялся вовсе (найдено при проверке
   2026-08-03).
