...
слот кампании. Любая локальная копия рано или поздно разъедется с реальной
громкостью.
...
- Не хранить громкость в переменных виджета и не писать её в `GameUserSettings`
  напрямую: звук идёт через `UTacticsAudioSubsystem`, состояние — в слоте.
