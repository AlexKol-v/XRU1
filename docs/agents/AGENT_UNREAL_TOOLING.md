...
- ⚠️ **Кириллица в теле скрипта теряется** (2026-08-04). Мост сохраняет
  присланный текст в `Content/UnrealClaude/Scripts/*.py` в системной кодировке,
  и русские строки доезжают до редактора как `?????` — молча, без ошибки.
  Заметно это только по результату: субтитры в собранном ассете оказались
  вопросительными знаками. Решение: держать тексты в отдельном **UTF-8** файле
  и читать их из скрипта явным `io.open(path, encoding='utf-8')`, а сам скрипт
  писать в ASCII. То же касается `print()` с русским текстом — в лог уйдут `?`.
...
### 5.2.9 Материалы из Python: три ловушки (2026-08-03)

Собран `M_PP_FogOfWar` целиком скриптом (`MaterialEditingLibrary`), по дороге
выяснилось:

- **Пересборка поверх существующего материала оставляет мусор.**
  `delete_all_material_expressions` чистит НЕ ВСЕ ноды (проверено: 6 из 14
  остаются), а добраться до остатков нечем — `expression_collection` в Python не
  экспонирована. Чистый граф получается только созданием с нуля, то есть
  удалением `.uasset` при ЗАКРЫТОМ редакторе (загруженный материал удерживают
  native-ссылки, `delete_asset` открывает модалку «используется»).
- **⚠️ Удаление ассета обнуляет ссылки на него.** После удаления `.uasset`
  редактор при старте пишет `LoadErrors: … отсутствует пакет …`, и свойство,
  которое на него ссылалось (у нас `FogOfWarMaterial` на пешке-камере), молча
  становится `None`. Материал надо ПЕРЕНАЗНАЧИТЬ после пересоздания.
- **Порядок свойств значим.** Домену `Deferred Decal` допустимы только
  Translucent/AlphaComposite/Modulate, поэтому `blend_mode` ставится ДО
  `material_domain` — иначе промежуточное состояние роняет компиляцию. Так же и
  с типами: у арифметических нод нет именованных выходов `R`/`G` (нужен
  `ComponentMask`), а `SceneTexture` даёт float4 — умножение на float3 не
  компилируется.

Запись свойства на CDO Blueprint надёжнее делать Python-ом с `modify()` +
`compile_blueprint` + `save_asset(only_if_is_dirty=False)`: `set_asset_property`
через мост меняет CDO в памяти, но значение может не дойти до живого актора.
