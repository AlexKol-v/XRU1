...
Снимок актуализирован 2026-07-28. C++-часть A16 (fire/reaction context,
mechanics-only commit, cover anchor/StepOut, death ownership, action barriers и
общий enemy planner) собрана на UE 5.7. Интеграция **ещё не завершена**:
сохранённые fire/death Blueprint-графы используют старые hooks, поэтому выстрел
watchdog-отменяется с возвратом AP. Ручной следующий шаг —
`17_MANUAL_EDITOR_CHECKLIST.md`; исходные доказательства — аудит 16.
...
| `UCoverDetectionComponent` | укрытия трейсами, GE-теги Cover.*, бонус 20/40, единый `CoverTuning`; active wall теперь стабилизирован latch/hysteresis | `ActiveCoverAnchor/Normal/WallId/Revision`, `OnActiveCoverChanged`, `MatchesActiveCoverHit`, `RequestActiveCoverReselection`, target-aware `FindPeekEdgeSide` |
...
| `AUnitBase` | статы/life state/visual snapshot; разделяет firing eye и presentation root, удерживает cover yaw при aim, сериализует hug/turn; DeathMontage единолично владеет death pose | `GetFireMontageFor(Target, Stance, Eye, Root)`, `PreviewAimAtTarget`, `NotifyDeathMontageFinished`, `HugCover`; пять BP death callbacks ещё подключить по доку 17 |
...
| `UGA_Attack` | immutable fire transaction: AP reservation, frozen target/chance/origin/stance/root, watchdog/refund и terminal barrier | `OnFireActionStarted`, `GetFireActionPresentation`, `IsFireActionCurrent`, notify-only `FireCommit`, `Complete/AbortFireAction`; сохранённый BP ещё мигрировать по доку 17 |
| `UGA_Overwatch` | долгоживущий parent + вложенный reaction action, глобальный reaction slot, pause mover/camera/slow-mo до terminal | `OnReactionActionStarted`, `GetReactionActionPresentation`, `IsReactionActionCurrent`, `FireCommit`, `Complete/AbortReactionAction`; сохранённый BP ещё мигрировать |
...
| `UTacticsCombatStatics` | общий выстрел/проверки + **занятость** (навмеш статичен, юниты — диски `UnitObstacleRadius` = 60 см на уровне запросов, как occupancy-клетки XCOM) | `ResolveShot` (форс 100/0 для туториала; шумит врагам), `ComputeHitChance` (укрытие + hunker ×2), `HasLineOfSight`, `SquadHasLineOfSight`, `IsUnitAlive/Downed/Evacuated`, `NotifyCombatNoise`, `GetPointAlongPathBudget`; занятость: **`GetUnitClearance`** (просвет = занятая клетка + радиус капсулы бегущего — единая величина на «встать» и «пройти», см. §2.5), `GetUnitObstacles` (бегущие исключаются — диск встанет по финишу; «бегущий» = `IsUnitInTransit`, по статусу path following, НЕ по velocity), `AdjustGoalOutOfUnits` (выталкивание цели приказа на край диска). **Ф3–Ф5 (2026-07-23):** `GetCoverTuning(World)` (тюнинг с GameInstance/CDO); `HasLineOfSightFromLocation(..., Shooter=nullptr)` — быстрый путь (как раньше) + запасной (огневые позиции у краёв); **`GetFiringPositions`** (единый источник позиций выглядывания, зовётся дважды с переставленными аргументами — стрелок и цель, §III.1/§III.2; всегда непустой, порядок центр→step-out→края); **`GetFiringStance`** (Open/OverCover/StepOut + точка выстрела, для Ф10/Ф11); enum `EFiringStance` в `CoverTypes.h`; диагностика — CVar **`xru1.LOS.Debug`** (лог + `DrawDebugSphere`). **Цикл 17 (2026-07-25):** **`GetShotGeometryObjects()`** — ЕДИНОЕ определение «геометрии, которая останавливает пулю и образует укрытие» (WorldStatic + WorldDynamic, без юнитов); им пользуются LOS, огневые позиции, стойка, все трейсы укрытия, прилипание AI к стене и подбор ракурса камеры |
| `AUnitAIController` | perception/utility AI; combat movement теперь получает occupancy-aware plan от общего визуализатора и исполняет `MoveAlongRoute`; AI attack step ждёт terminal fire/reaction action | `ExecuteUnitTurn`, `MoveWithBudget`, `MoveAlongRoute`; runtime обход союзников подтвердить PIE после BP fire migration |
| `ATacticalPlayerController` | единый command/action/reaction barrier; AP=0, selection, end turn и auto-advance ждут terminal; reaction сохраняет прежний global dilation и reconciles lethal selected unit | `CanIssueCommand`, `IsUnitActionInProgress`, `TryBeginReactionShot`, `EndShotPresentation`, `EndReactionShotPresentation` |
| `ATacticalCameraPawn` | XCOM-камера: панорама/поворот 45°/зум/**полёт к цели**; `TacticalYaw/TacticalZoom` — постоянный пользовательский ракурс, `TargetYaw/TargetZoom` — временная цель action-camera. Выбор/фокус меняют только позицию, aim/shot после себя возвращают глобальный ракурс; `IsPlayingShotFrame` отличает конечный выстрел от бессрочного aim-frame. **Цикл 17 (2026-07-25):** кадр прицеливания считается как ПОЗИЦИЯ КАМЕРЫ + ТОЧКА ВЗГЛЯДА в мире, параметры пружины выводятся из них; высота пивота — явное состояние `PivotWorldZ` (раньше кадр считался от Z стрелка, а строился от Z пешки, которая не двигается вообще); плечо задано в СМ, а не в градусах; лестница кандидатов гарантирует видимость цели | `AddPanInput/AddRotationStep/AddZoomInput`, `FocusOnActor`/`FocusOnLocation`, `SetFollowTarget`/`ClearFollowTarget`, `FrameShot`, `IsPlayingShotFrame`; внутри — `FitSubjectsInFrame`, `PullCameraOutOfGeometry`, `ScoreShotCandidate` |
...
- AI поле не строит: `MoveWithBudget` берёт funnel budget point и вызывает
  `MoveToLocation`. Старый комментарий об усечении у стоящего юнита неверен —
  эта логика удалена из `GetPointAlongPathBudget`; путь может пройти сквозь
  союзника, после чего Crowd упирается. PIE 2026-07-28 подтвердил блокер.
  Вынести общий planner/model и вести enemy через тот же occupancy-aware
  `MoveAlongRoute` — A16-P3.
...
знает» (перцепция живёт между ходами), а ВНУТРИ Combat действие выбирается
**утилити-скорингом**.

> ⚠️ Раздел переписан 2026-07-24. Полное обоснование архитектуры — **ADR-1** в
> [13_AI_STATE_MACHINE_PLAN.md](13_AI_STATE_MACHINE_PLAN.md), правила XCOM с
> исходными весами — [12_AI_REFERENCE.md](12_AI_REFERENCE.md). Здесь — только
> карта кода.

**Каркас решения** (`AIDecisionTypes.h`, `AIActionEvaluator.h`):
`BuildDecisionContext` собирает снимок мира на активацию → `DecideAction`
перебирает `ActionEvaluators` → `ExecuteDecision` исполняет `FAIDecision`.
Перебор идёт по убыванию `GetMaxPossibleScore()` и обрывается, как только
лучший результат достиг потолка следующего кандидата, — дорогой
`FindCoverPoint` не считается, когда есть уверенный выстрел. **Новое поведение
= новый наследник `UAIActionEvaluator`, существующие не правятся.**

Набор (`AIActionEvaluators.h`), потолки скора: `Retreat` 120 → `Shoot` 100
(масштабируется качеством выстрела: уверенный ×1, средний ×0.55, плохой ×0.15)
→ **`HunkerDown` 85** → `MoveToCover` 80 → `AdvanceToCover` 40 →
**`Overwatch` 30** → `CloseDistance` 20 → `Skip`.

Оборонительные оценщики добавлены в W2 (2026-07-25) — ровно наследником и
строкой в набор, существующие не правились: `HunkerDown` (мало HP + укрытие
работает против угрозы + `BestCoverAround != None` + ответить нечем),
`Overwatch` (врага видно, выстрела нет; рандомизация `ActivationChance = 0.33`
детерминированным зерном «юнит + кадр», чтобы `ScoreAction` оставалась
воспроизводимой). Исполняет их `TryActivateSelfAbility`: успех = СПИСАННЫЕ AP,
а не возврат `TryActivateAbilityByClass`.

⚠️ **Стоящие юниты путь не блокируют** (правило XCOM, исправлено 2026-07-25):
`GetPointAlongPathBudget` больше не урезает маршрут по клиренсу до союзника, а
`FindPathClearanceLimit` удалена. Прежнее усечение делало недостижимой любую
точку за спиной товарища, и боты выстраивались в колонну. Занятость проверяется
на КОНЦЕ маршрута (`AdjustGoalOutOfUnits`, отбор кандидатов в `FindCoverPoint`).

**Выбор цели** — `ScoreTarget`: шанс попадания доминирует (70/40/10), провокация
танка — приказ (GDD §7), фланг +50, добивание +15, недостижимая цель −200.

`FindCoverPoint(PathBudget, bRetreat, bAdvance, OutDetails)`: кольцевой
сэмплинг (4 радиуса × 12 углов); открытый кандидат «прилипает» к стене между
ним и угрозой. Каждая точка оценивается **по каждой угрозе отдельно** — от кого
закрыт, кого вижу, кого фланкирую (`FAICoverPointResult`), тем же LOS, что
решает выстрел. Достижимость проверяется ПЕРЕД оценкой (она дороже). Манёвр
только при выигрыше > `RelocateBias`. Веса — `Tactics|AI|Weights`.

**Укрытие и фланг = физика выстрела**: `GetCoverAgainst` пускает толстый луч от
цели к ФАКТИЧЕСКОЙ огневой позиции стрелка (с выглядыванием) на высотах
half/full. Углов и дуг нет. `CoverSides`/`BestCoverDirection` — отдельный
**визуальный** слой для анимации и отладки, на фланг не влияет.

Диагностика: `xru1.AI.LogCombat 1` — все кандидаты со скорами, расшифровка
выбранной точки, сверка «встал туда, куда решил»; `xru1.LOS.Debug 1` — огневые
позиции, стороны укрытия и линии «укрытие держит / фланг» до каждого врага;
`xru1.Cover.Debug 1` — по каждому запросу укрытия строка
`[Cover] цель vs стрелок: distToShot / traceLen / cover / blocker` и отрисовка
самого луча: видно не только ЧТО решили, но и ПОЧЕМУ (какой актор засчитан
стеной). Заведён в цикле 17, когда «стеной» оказался сам стрелок.
...
> решения одноразовые на активацию в пошаговом ходе. Условие пересмотра
> записано в ADR-1.5; утилити-слой переносится в ST 1:1, потому что оценка
> отделена от исполнения.
