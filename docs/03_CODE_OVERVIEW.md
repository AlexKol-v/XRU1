...
| `UCoverDetectionComponent` | укрытия трейсами, GE-теги Cover.*, бонус защиты 20/40. **Ф2 (2026-07-23):** высоты меряются от точки ПОЛА (`Base = ActorLocation − CapsuleHalfHeight`), а не от центра капсулы. **Ф3:** геометрия/числа укрытия переехали в `UCoverTuningDataAsset` (поля компонента удалены), чтение — `GetTuning()` (`TuningOverride` → глобальный → CDO). **Цикл 17 (2026-07-25):** трейсы укрытия перешли с КАНАЛА на object-query линии огня (юниты больше не укрытие) и обрезаются дистанцией до стрелка (стена обязана быть МЕЖДУ) | `EvaluateSurroundings`, `GetCoverAgainst`, `GetDefenseBonusAgainst`, `EvaluateCoverAtLocation` (Base = пол), static `TraceCoverAtLocation` (без параметра-канала, есть выдача `FHitResult`), static **`GetCoverTraceLength`**, `GetTuning`, **`FindPeekEdgeSide`** (2026-07-27: с какой стороны КОНЧАЕТСЯ укрытие — ось выглядывания для ABP, питает `FUnitVisualState::PeekSideLocal`), `OnCoverStateChanged` (стойки в ABP) |
...
| `AUnitBase` | статы (`BaseMaxHealth`/BaseAim/ShotDamage/AttackRange/MoveRange, Squadsight, bCanBeDowned), классы способностей юнита, **смерть/Downed/эвакуация**, PatrolPoints, **подсветка** (кольцо-декаль `SelectionDecal` + обводка Custom Depth: stencil 1 = отряд, 2 = враг); `PostInitializeComponents` переносит классовый `BaseMaxHealth` в GAS `Health/MaxHealth` **до** StartupEffects и создания HUD (Assault/Sniper/Healer/Tank = 100/80/90/150); нативные defaults дают Attack/Overwatch/Hunker даже новому BP; в `BeginPlay` реальный `AUnit_*` нормализует `UnitRole` и восстанавливает пустой/скопированный позывной (`Клин/Оса/Шприц/Молот`); в конструкторе включён **`bUseAccelerationForPaths`**; **навмеш юнит НЕ трогает** | `SetDowned`/`ReviveFromDowned`, `Evacuate`, `SetSelectionHighlight`/`SetHoverHighlight`, BP-хуки `OnDied/OnDownedChanged/OnEvacuated`; `NotifyUnitStateChanged` централизованно обновляет HUD после life-state, Hunker/Taunt/Overwatch и start/finish движения; для ABP (S2): **`GetVisualState`** (единый срез позы — `EUnitPose`, укрытие, `CoverDirectionLocal`, `PeekSideLocal`, `PendingTurnYaw`), **`GetFireMontageFor`** (монтаж + `EFiringStance` + точка выстрела), **`HugCover`** (разворот лицом к стене + ПОДШАГ вплотную; и то и другое плавно через `CharacterMovement` в `Tick`, поэтому играет обычная локомоция, а `bMoving` на время подшага = true), **`FaceTowardsSmooth`** (плавный доворот на месте, полная дельта → `PendingTurnYaw` для выбора клипа `Turn_045..180`; зовётся при наводке на цель из `ATacticalPlayerController`); для HUD: `UnitDisplayName`, `GetAbilityUsesRemaining` |
...
| `UGA_Overwatch` | реакционный выстрел в чужую фазу (штраф −10 поверх общего расчёта точности), тег State.Overwatch, самозавершение; право на реакцию — **общий предикат `CanTargetActor`** (2026-07-20: радиус перцепции шире дальности оружия, раньше юнит реагировал на недостижимые цели). **W1 (2026-07-25): ДВА триггера** — старый `OnTargetPerceptionUpdated` («враг появился») и новый периодический опрос ДВИЖУЩИХСЯ врагов (`CheckMovingTargets`, `ReactionCheckInterval`/`ReactionMinTravel`). Второй — надмножество первого; без него уже видимый враг не давал стимула вообще, и овервотч бота не срабатывал НИКОГДА. Оба идут через единый `TryReactTo` (фаза, лимит, `CanTargetActor`, «по этому перемещению уже стреляли») | BP-хук `OnReactionShot` |
