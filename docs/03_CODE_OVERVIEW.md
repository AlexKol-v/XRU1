...
**Камера: наклон и высота обзора принадлежат только кадру выстрела.** У игрока
нет ввода, который бы их менял, поэтому `AbandonShotFraming` возвращает их
ВСЕГДА (иначе камера зависала «в режиме атаки», когда кадр бросали посреди —
например, ход врага кончился в кадре его выстрела). Yaw/зум/позиция —
игроковы оси: бросаются как есть, их уже ведёт новый интент.
