...
| `UFogGridSubsystem` | ВИЗУАЛЬНЫЙ слой тумана: сетка `Unknown/Explored/Visible`, запекание блокеров, DDA-растеризация и текстура для `M_PP_FogOfWar`. Подписан на пересчёт правил и ничего не решает сам ([10_FOG_OF_WAR §6](10_FOG_OF_WAR.md)) |
| `UFogOfWarConfigDataAsset` | тюнинг сетки и вида тумана (клетка, проба блокера, яркости состояний); правил не настраивает |
...
- порядок фаз зависит от стойки: **Open** — доворот до montage (боец и так на
  ногах, вставать не надо), **StepOut** — доворот после прибытия на огневую
  точку, **OverCover** (любой боец в позе укрытия, см. §5.1) —
...
- **страховка «не стрелять в спину» живёт на самом выстреле**, а не до него:
  `EnsureFacingAtCommit` в `FireCommit` обеих способностей доворачивает мгновенно
  только если остаточное отклонение больше `AimSnapMaxError` (8°), и пишет об
  этом Warning — в штатном выстреле строки быть не должно.

  ⚠️ Узлов `Face Actor Towards` в BP_GA_Attack/BP_GA_Overwatch больше НЕТ (было
  по два в каждом). Они стояли между фазой доворота и montage и потому ломали
  единственную стойку, где доворот идёт уже во время анимации: корпус щёлкал
  до подъёма, и «встал → довернулся → выстрелил» превращалось в «щёлкнул →
  встал → выстрелил». Заметнее всего это было в наблюдении, где перед montage
  нет камерной паузы. Возвращать этот узел в граф нельзя.

**Ритм презентации нормирован по стойкам** (числа — от команды игрока):
...
лежит `MM_Rifle_DryFire` (стрельба стоя), поэтому «вставание» целиком делает
переход из позы укрытия. При blend-in 0.10 с боец вставал мгновенно — отсюда
«встал и выстрелил за четверть секунды». Текущие значения: blend-in 0.35,
blend-out 0.25, `RateScale` 0.8 (0.80 → 1.00 с), выстрел на 0.50 в пространстве
монтажа = 0.625 с реального времени. У `AM_Fire_Open` — 0.20 / 0.15 / 1.0,
выстрел на 0.24.

⚠️ **Длительность подъёма задают ДВА числа, и они обязаны совпадать**: blend-in
монтажа (0.35) и переход стейт-машины `ABP_Solider.Locomotion`
`CrouchCover → Idle` / `HighCover → Idle` (тоже 0.35). Montage играет в Default
Slot ПОВЕРХ позы, поэтому фактическая скорость вставания — это быстрейшее из
двух: пока переход стоял на 0.20, боец успевал выпрямиться до того, как
анимация набирала вес, и подъём читался рывком, хотя монтаж был перетаймлен
правильно. К этой же паре привязан `AimTurnRiseDelay` (0.35) — доворот
планируется от конца подъёма. Меняешь одно — меняй все три. Обратные переходы
(посадка, `Idle → CrouchCover/HighCover`) остаются 0.20: садиться быстрее, чем
вставать, — нормально, и посадка идёт уже под уход камеры.

**Посадка обратно в укрытие ждёт камеру.** Montage кончается вскоре после
выстрела, и поза сама вернулась бы в присед — боец садился бы, пока кадр ещё
держится. Поэтому на время презентации у стоек **OverCover и StepOut** взводится
`AUnitBase::SetPresentationStanding(true)`: поза остаётся `Stand` поверх поз
укрытия, а снимает флаг терминал (`ReleasePresentationStanding` рядом с
`EndShotPresentation`/`EndReactionPresentation`) — то есть после удержания кадра.
Смерть, ранение, движение и наблюдение флаг не перебивает.

⚠️ StepOut попал сюда по той же причине, что и OverCover: боец добегает до
огневой точки, компонент укрытий находит там стену, поза возвращается в
`Crouch`/`High` — и выстрел уходит из приседа (лог PIE 2026-08-04, выстрелы
4/22/38: `стойка=StepOut` при `[FireCommit] … поза=2/3`).

### 5.1. Стойка выстрела решается ПОЗОЙ, а не направлением стены

`UTacticsCombatStatics::FindFiringSolution` (стойка + точка выстрела) для решения
из центра/step-up спрашивает `IsUnitInCoverPose` — тот же `FUnitVisualState`,
который читает Anim Blueprint:
...
Прежнее правило смотрело на `GetCoverAgainst`-подобный трейс МЕЖДУ стрелком и
целью: боец за стеной, которая смотрит вбок или назад, получал `Open`
(«стрелять можно с места, вставать незачем»). Но стрелковый montage анимирован
СТОЯ, а поза оставалась `Crouch` — очередь уходила из приседа «в пол». В логе
PIE 2026-08-04 это 18 выстрелов из 43 (`[FireCommit] … поза=2/3`). Отдельного
клипа стрельбы из приседа нет и не планируется, поэтому единственная честная
презентация из укрытия — подняться.

### 5.2. Одна истина на вопрос «могу ли я выстрелить»

`FindFiringSolution` — **единственный** предикат доступности выстрела. Его
зовут `UGA_Attack::GetTargetStatus` (а через него — HUD, подсветка целей,
...
Согласованность держится **по построению**, а не по совпадению правил:

- позиция принимается ровно тем предикатом, которым потом проверяет commit —
  `HasLineOfSightFromFrozenOrigin(позиция, цель)`. Свой перебор по заранее
  построенному набору точек цели здесь был неверен: набор строится ОТ точки
...
- набор огневых позиций замкнут корпусной точкой стрелка — зеркалом корпусной
  точки цели из `GetTargetExposedPoints` (Ф5). Без неё «стрелок.глаза →
  враг.корпус» проходило, а обратное — нет, и боец стрелял в того, кто ответить
  не мог. Идёт последней: выйти краем лучше, чем бить от бедра;
- быстрый путь (центр глаз против глаз/корпуса цели) — строгое подмножество
  того же перебора: он экономит nav-проекции на каждом опросе HUD/AI, но не
  может дать решение, которого не дал бы полный перебор.

⚠️ Раньше истин было ДВЕ: доступность решала `HasLineOfSight` (вопрос
ВИДИМОСТИ, более широкий набор точек — в т.ч. корпусная точка стрелка, которой
нет среди огневых позиций), а активация — линию огня из замороженной точки.
Расхождение видно в логе PIE 2026-08-04: HUD показывал шанс, игрок жал выстрел
и получал `[FireAction] Reject at activation: из замороженной позиции нет линии
огня` (Assault → Marauder_12 дважды подряд), а AI на том же расхождении терял
ход целиком (`атака не активировалась — цель заблокирована до конца хода`).
Заводить рядом второй предикат линии огня нельзя — `HasLineOfSight` остаётся
только для ОБНАРУЖЕНИЯ (туман войны, squadsight-видимость союзников).

**Отказ бесплатен только ДО анимации.** `NotifyPresentationMontageStarting`
возвращает `bool`: перед самым стартом montage замороженное решение проверяется
ещё раз (`IsFrozenPresentationSolutionValid` → `IsFrozenFireCommitValid` /
`IsFrozenReactionCommitValid`), и если цель успела уйти из линии огня — ветка
презентации обрывается, транзакция отменяется, анимация не играется. Раньше
проверка стояла только на самом `FireCommit`: боец вскидывался, стрелял — и
лишь тогда решение отклонялось (`[ReactionAction] Reject invalid frozen
solution`, лог PIE 2026-08-04). У наблюдения это норма жизни: цель бежит всё
окно реакции. Выстрел наблюдения при этом НЕ сгорает — лимит растёт только
после состоявшегося `FireCommit`.

**Squadsight — тоже одно определение.** `UGA_Attack::IsSquadsightShot` («цель
дальше собственного обзора `SquadVisionRange`, стрелок владеет Squadsight»)
решает и допуск цели, и метку транзакции, и штраф к точности (GDD §5.4, −10).
Штраф раньше висел на условии `!HasLineOfSight` и был недостижим:
`ComputeEffectiveAim` зовётся только после `CanTargetActor`, а тот требует
геометрию всегда — то есть −10 не применялся ни разу вопреки GDD.

⚠️ `FindFireCommitTime` делит время notify на `RateScale`: планировщик доворота
и ожидание кадра работают в РЕАЛЬНЫХ секундах, а notify живёт в пространстве
монтажа. Меняя темп монтажа, ничего в коде править не нужно — но и забывать про
это деление нельзя.

`MinMontageLeadIn` — нижняя граница: анимация поверх укрытия сама даёт кадру
0.40 с до выстрела, и без этой границы ожидание обнулялось целиком, отчего
выстрел с места оказывался вдвое короче выхода из-за угла.
...
редакторе Niagara компилирует скрипты лениво — к первому спавну, поэтому первый
выстрел в PIE стоил фриза (в записи 2026-08-03 прямо в кадре выстрела шли три
компиляции: `NS_MuzzleFlash` 0.17 с, `NS_BulletTracer` 0.05 с,
`NS_Impact_Concrete` 0.19 с). Прогрев переносит цену на старт боя; в упакованной
игре скрипты уже скомпилированы, и метод там пустой.
...
### 9.4. Боевая миссия: расстановка данными и реплики-реакции

Четыре класса, появившиеся при сборке Mission01
([16_MISSION01_ASSEMBLY.md](16_MISSION01_ASSEMBLY.md)):

| Класс | Ответственность |
|---|---|
| `ATacticalEncounter` | стартовая группа врагов, описанная ДАННЫМИ: состав по сложности, под или одиночки, точки спавна, маршрут патруля, сторожа. Спавнит до `StartCombat`, поэтому бойцы попадают в стороны обычным сбором |
| `ATacticalReinforcementBeacon` | волна подкрепления по модели XCOM 2: маяк виден сразу, высадка через `Countdown` ходов врага, прибывшие знают позицию отряда; триггер — «живых врагов ≤ порога» либо StateTree-задача `Call Reinforcements` |
| `UMissionVoiceDataAsset` + `UMissionVoiceDirectorSubsystem` | таблица реплик боя и её проигрыватель: слушает `Quest.Event`, применяет правила (`bOncePerMission`, кулдаун, приоритет, самый ранний ход) и отдаёт субтитр общему слою |
| `xru1.Mission.Validate` | проверка расстановки: длины пути ПО НАВМЕШУ в ходах, поды, маршруты, потерянные ссылки на точки, актор в чужом уровне |

**Разделение StateTree и реплик — принятое решение, а не удобство.** Цели
последовательны, у каждой есть «выполнено» — это StateTree. Реакции боя
происходят в произвольном порядке и могут не случиться вовсе — это таблица.
Попытка держать реакции фоновыми задачами родительского состояния дала 101 472
перезапуска дерева за прогон: состояние без задач, учитываемых для завершения,
закрывается сразу. Лог входа в состояние с тех пор фильтруется — частые
перезаходы печатают одну строку с диагнозом вместо потока одинаковых.
