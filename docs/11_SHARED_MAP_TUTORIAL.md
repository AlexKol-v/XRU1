...
> **Собрано и пройдено (2026-08-04).** Актуальное состояние, таблица реплик и
> разбор прогонов — [16_MISSION01_ASSEMBLY.md §6.2](16_MISSION01_ASSEMBLY.md).
> Важное отличие от плана ниже: **реплики миссии живут не в дереве**, а в
> таблице `DA_Voice_Mission01`, которую проигрывает
> `UMissionVoiceDirectorSubsystem`. Состояние, у которого все задачи фоновые,
> закрывается сразу и перезапускает дерево — проверено прогоном.

Исходная рекомендуемая структура (цели):
