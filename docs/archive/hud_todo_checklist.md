# Архив — боевой HUD: пошаговый чеклист для редактора

> **⚠️ АРХИВ (2026-07-21): этап 6 закрыт.** Сборка HUD выполнена, чеклист
> исторический — оставлен как справка «как это собиралось». Актуальный
> статус проекта — [`../09_GAMEPLAY_STATUS.md`](../09_GAMEPLAY_STATUS.md).
...
> `tactical_hud_build_guide.md`.** Ниже — краткая выжимка для сверки.
...
- [x] **Чистка WBP-графа от перенесённого** (агент через UnrealClaude MCP,
  готовый план с GUID нод — `sonnet_mcp_hud_tasks.md`, выполнено 2026-07-20):
...
- [x] Всё проходит мышью, без клавиатуры — подтверждено ручным PIE-прогоном
  пользователя 2026-07-21 (см. `../08_HANDOFF.md`)
- [x] Чекбоксы: `04_ROADMAP.md` этап 6 (WBP_TacticalHUD) и этап 4 («HUD юнита»)
- [x] `05_EDITOR_GUIDE.md` сверен при реструктуризации доков (2026-07-21)
- [x] Этот файл переведён в архив (реструктуризация доков, 2026-07-21)

Диагностические логи `[Unit] …` в `AUnitBase` ещё не удалены — перенесено в
`../09_GAMEPLAY_STATUS.md` §D как мелкую незакрытую задачу.
