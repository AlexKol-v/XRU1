...
>
> **Снимок 2026-07-28:** stable active cover, target-aware root StepOut и общий
> enemy planner реализованы в C++ и собраны; runtime-приёмка заблокирована старым
> fire BP. Сначала выполнить [17_MANUAL_EDITOR_CHECKLIST.md](17_MANUAL_EDITOR_CHECKLIST.md),
> затем возвращаться к оставшимся Ф11/Ф12 и stuck/replan.
...
## Ф0 — Контент карты (предусловие) `[x]` ГОТОВО
> **Статус (2026-07-24):** закрыто. Прямого отчёта по расстановке не было, но
> Ф2/Ф4/Ф5 приняты игроком в PIE, а их критерии (тег `Cover.Half` у ящика,
> `Cover.Full` у стены, «оба за узкими укрытиями напротив») без расставленных
> укрытий проверить нечем — значит контент на карте есть.
> Если позже понадобится ДОБАВИТЬ укрытий — полосы высот и обязательные сцены
> ниже остаются в силе.
...
## Ф2 — Высоты укрытий от ПОЛА, а не от центра капсулы `[x]` ГОТОВО, принято игроком
> **Что сделано:** `TraceCoverInDirection` считает `Base = ActorLocation −
> CapsuleHalfHeight` (точка пола, хелпер `OwnerCapsuleHalfHeight`);
> `FindCoverPoint` передаёт `Projected.Location` в `EvaluateCoverAtLocation`
> без `+ CapsuleHalfHeight`.
...
## Ф3 — Data Asset тюнинга `[x]` ГОТОВО, принято игроком
> **Что сделано:** `UCoverTuningDataAsset` со всеми полями §III.6 (дефолты =
> прежним числам); резолвер `UTacticsCombatStatics::GetCoverTuning(World)` с
> фолбэком на CDO; `UTacticsGameInstance::CoverTuning` и пер-юнитный
> `UCoverDetectionComponent::TuningOverride` + `GetTuning()`. Мигрированы 6 полей
> компонента, `static constexpr` (`EyeHeightOffset`, `LosPeekOffset`,
...
> `ComputeEffectiveAim`. Ассет `Content/XRU1Game/Data/DA_CoverTuning` создан и
> назначен в `BP_TacticsGameInstance`.
...
## Ф4 — Огневые позиции стрелка (peek) `[x]` ГОТОВО, принято игроком
> **Что сделано:** `EFiringStance` в `CoverTypes.h`; `GetFiringPositions`
> (всегда непустой список: центр→быстрый step-out→края укрытия; порядок значим);
> `HasLineOfSightFromLocation` получил `const AActor* Shooter = nullptr` и
> запасной путь (быстрый путь сохранён дословно); `HasLineOfSight` пробрасывает
...
## Ф5 — Симметричное выглядывание: цель тоже высовывается `[x]` ГОТОВО, принято игроком
> **Что сделано:** в запасном пути `HasLineOfSightFromLocation` — второй вызов
> **той же** `GetFiringPositions(World, Target, глазаЦели, EyeLocation,
> PositionsB)` + точка корпуса цели (`TargetPoints[1]`), перебор пар (позиция
> стрелка × позиция цели). Новой функции и новых порогов не заведено. Укрытие
> цели по-прежнему считается от её НАСТОЯЩЕЙ позиции в `ComputeHitChance`
> (peek — только для луча).
...
> **Статус:** закрыто. `GetTargetStatus`, `CanTargetActor`, `GetAttackTargets`,
> `ResolveShot` все идут через единый `HasLineOfSight`, поэтому peek/симметрия
> Ф4/Ф5 доходят до выбора целей и выстрела автоматически.
...
## Ф7 — Глухая оборона требует укрытия `[x]` ГОТОВО, принято игроком
> **Что сделано:** `UGA_HunkerDown::CanActivateAbility` требует тег
> `Cover.Half`/`Cover.Full` (без укрытия активация отклоняется, AP не тратится).
> HUD: в `CanIssueCommand` case `HunkerDown` — проверка
> `BestCoverAround != None`, кнопка серая без укрытия.
...
## Ф8 — Flanked / жёлтый щит `[x]` СОБРАНО (ядро + UI)
> **Статус (2026-07-24): реализовано и собрано целиком** (`Result: Succeeded`).
>
> **Ядро:** `EvaluateSurroundings` перебирает `SurroundingDirections` из
> `DA_CoverTuning` (дефолт **8**, поле заводилось в Ф3 как задел и до правки ни
> на что не влияло) + ранний выход при найденном Full;
> `UTacticsCombatStatics::IsTargetFlankedBy` по §III.4, без кэша.
>
> **UI:** введён `ECoverShield` (None/Covered/Flanked) в `CoverTypes.h` и
> ЕДИНАЯ функция `UTacticsCombatStatics::GetCoverShieldAgainst(Target, Shooter,
> OutShieldCover)` — все поверхности считают три состояния ею, а не собирают
> из bool'ов каждая по-своему. В теме HUD — `FlankedCoverIcon`,
> `FlankedCoverTint` (жёлтый), `CoveredCoverTint`, плюс
> `GetCoverShieldIcon`/`GetCoverShieldTint`. Панель цели показывает третье
> состояние; `UCoverIconWidget` получил режим `bShowFlankedWhileTargeting` —
> пока игрок целится, иконка над головой считается ПРОТИВ ВЫБРАННОГО СТРЕЛКА,
> вне прицеливания возвращается к локальному `BestCoverAround`.
>
> ⚠️ **Ожидаемый побочный эффект, НЕ баг:** укрытий находится больше (углы,
> диагонали) → чаще доступен hunker (Ф7), чаще жёлтый щит вместо «открыт».
>
> **Текстуру заводить НЕ обязательно.** Без своей `FlankedCoverIcon` берётся
> обычный щит укрытия, покрашенный в `FlankedCoverTint` — жёлтый щит виден в
> игре сразу. Отдельный арт можно добавить позже, ничего не переписывая.
>
> ⚠️ **Где искать поля.** Они в UI-теме `UTacticalHUDStyleData` (категория
> «02. Статусы|Укрытия»), а НЕ в `DA_CoverTuning`: тот про геометрию укрытий
> (высоты, дистанции, peek), а не про арт.

Всё сделано:
- ✅ `EvaluateSurroundings`: 4 → `SurroundingDirections` (8 с диагоналями), §II.6 п.4.
- ✅ `IsTargetFlankedBy` (§III.4) + `ECoverShield`/`GetCoverShieldAgainst` —
  ЕДИНЫЙ источник трёх состояний щита для всех поверхностей.
- ✅ **Иконка над головой** (`UCoverIconWidget`, флаг `bShowFlankedWhileTargeting`):
  на время прицеливания считает щит против выбранного стрелка, вне
  прицеливания — прежнее локальное `BestCoverAround`.
- ✅ **Панель цели** (`TacticalHUDWidget`): третье состояние.
- ✅ **Текстура в `UTacticalHUDStyleData`** (`FlankedCoverIcon` +
  `FlankedCoverTint`); без своей текстуры работает тинтом.
- Иконка в карточке отряда (`GetCoverIconForUnit`) намеренно осталась
  **локальной** — это статус своего бойца, per-shooter ей быть незачем.
- **Приёмка:** зайти сбоку от врага в укрытии → жёлтый щит **и над головой, и в
  панели цели**, шанс вырос.

> ⚠️ **Правило фланга с тех пор изменилось.** Ф8 определяла фланг как
> «`BestCoverAround != None`, но укрытия против стрелка нет». Само определение
> живо, а вот КАК считается «укрытие против стрелка» — переделано дважды
> (стороны+дуга, затем физика выстрела) и уточнено по геометрии в цикле 17.
> Актуальная модель и обоснование —
> [12_AI_REFERENCE §VIII.D п.8](12_AI_REFERENCE.md#d-осознанные-отклонения-от-xcom-фиксируем-чтобы-не-чинить)
> и циклы 10 / 17 в [13_AI_STATE_MACHINE_PLAN](13_AI_STATE_MACHINE_PLAN.md).
>
> 🔴 **Цикл 17 (2026-07-25) — почему фланг не срабатывал вплотную.** Трейсы
> укрытия шли по КАНАЛУ `ECC_WorldStatic`, а капсула и меш юнита этот канал
> **блокируют**: луч «от цели к стрелку» упирался в самого стрелка и всегда
> давал Full. Плюс луч не обрезался дистанцией до стрелка и ловил стену у него
> за спиной. Теперь геометрия укрытия — та же, что у линии огня
> (`UTacticsCombatStatics::GetShotGeometryObjects`, без юнитов), а длина луча
> ограничена `UCoverDetectionComponent::GetCoverTraceLength`. Диагностика —
> `xru1.Cover.Debug 1`.

## Ф9 — AI на тех же правилах `[~]`
> **Статус (2026-07-24):** сверено по коду — п.1 и п.3 уже реализованы.
> Остаток (п.2, скор против всех угроз) перенесён в
> [13_AI_STATE_MACHINE_PLAN.md](13_AI_STATE_MACHINE_PLAN.md) **фаза A5**, где
> он идёт вместе с весами фланга/высоты/кучности — по отдельности делать нет
> смысла. Правила, по которым это считается, — в
...
- ⏭ Скор укрытия — против **всех видимых угроз**: начать просто (укрытие против
  ближайших N, штраф если хоть от кого-то открыт). → фаза **A5**.
...
## Ф10 — Стойки и анимация выстрела из укрытия `[~]` — собрана, не принята

> S1/S2, `ABP_Solider` и монтажи физически реализованы. Аудит/PIE 2026-07-28
> показал, что одного `FUnitVisualState` недостаточно: StepOut, fire commit,
> orientation и return не имеют общей транзакции. Актуальная спецификация
> полностью перенесена в `16_UNREAL_MCP_TECH_AUDIT.md` A16-P1–A16-P2; текст ниже заменён,
> чтобы исторический план не использовался как инструкция.

- Half → crouch/OverCover; Full → standing lean либо StepOut; Open → обычный
  выстрел.
- StepOut сохраняет home anchor/occupancy и выполняется единым C++ action
  coordinator с отдельными eye/root points и обратным route.
- `FireCommit` Branching Point — единственный commit-сигнал; notify сам не
  наносит урон, активный coordinator один раз вызывает mechanics resolve.
- **Приёмка:** полная матрица аудита 16 §7, включая interrupt, смерть и
  заблокированный return path.
...
| **Стрелок вплотную к цели у стены (ближний бой)** | **жёлтый щит, шанс НЕ снижен: стена цели больше не «между»** | цикл 17 |
| **Союзник стоит в 1 м от бойца в чистом поле** | **щита нет, глухая оборона недоступна** (юнит — не укрытие) | цикл 17 |
| **Кадр прицеливания на цели в 1 м** | **обе фигуры читаются, цель не спрятана за спиной стрелка** | цикл 17 |
| **Цель за высоким укрытием / на уступе** | **камера приподнимается и показывает цель, а не стену** | цикл 17 |
| **Враг выбежал из-за угла посреди своего хода** | **камера подхватывает его на бегу, а не пропускает ход** | цикл 17 |
| **Овервотч по УЖЕ ВИДИМОМУ бойцу, который сдвинулся** | **реакционный выстрел проходит** (до W1 — не проходил никогда) | W1 |
| **Боец в поле зрения овервотча стоит на месте** | реакции нет | W1 |
| **Регрессия W1:** боец игрока в овервотче, враг выбегает из-за угла | по-прежнему стреляет (старый триггер жив) | W1 |
| **Одно перемещение — один реакционный выстрел** | не очередь | W1 |
| **Врагу некуда стрелять** | ИНОГДА (≈1/3) встаёт в наблюдение, иначе сближается | W2 |
| **Раненый враг в укрытии, ответить нечем** | уходит в глухую оборону, не бежит | W2 |
| **Три бота идут к отряду** | **расходятся по разным укрытиям, а не колонной друг за другом** | цикл 18 |
| **Расстановка отряда врага** | держатся на виду друг у друга, но не в одном укрытии (спред 576 см + сплочённость) | цикл 21 |
| **На Easy атакует не больше 4 врагов за ход**, на Medium — 6 | лишние встают в наблюдение / двигаются, но не стреляют | A8 |
| **Бот дошёл до места, откуда стреляли, никого не нашёл** | разворачивается туда и в половине случаев встаёт в наблюдение | цикл 21 |
| **Точка простреливается бойцом игрока в овервотче** | бот выбирает другую, если она не сильно хуже | A7 `SafeToMove` |
| **Срабатывание наблюдения** | бегущий ЗАМИРАЕТ, мир замедляется (0.66), камера показывает выстрел, затем он бежит дальше | цикл 25 |
| **Два бойца в наблюдении на одного бегущего** | стреляют ПО ОЧЕРЕДИ, а не залпом | цикл 25 |
| **Танк применил провокацию** | все враги в 25 м, у кого есть выстрел по танку, бьют именно его. В логе — `[ПРОВОКАЦИЯ активна]` | цикл 25 |
| **Кадр прицеливания в упор** | камера у плеча (≈85 см назад), стрелок рамкой в углу кадра | цикл 24 |
| **Смена цели / юнита** | камера НЕ перепрыгивает на другую сторону оси стрелок→цель (правило 180°) | цикл 24 |
...
## VI.3 Приёмка Ф0–Ф7 — ЗАКРЫТА (2026-07-24)

Ф0–Ф7 приняты игроком в PIE. Data Asset `DA_CoverTuning` создан и назначен в
`BP_TacticsGameInstance`, укрытия на `Lvl_TopDown` расставлены. Пошаговые
инструкции по редактору, жившие здесь, свою задачу выполнили и удалены —
актуальные требования к контенту остались в §Ф0 (полосы высот, узкие укрытия,
обязательные сцены), они понадобятся при расширении карты.

**Диагностика — по-прежнему рабочая, пригодится дальше:**
- `xru1.LOS.Debug 1` — лог `[LOS] shooter -> target: shooterPos=N targetPos=M
  visible=0/1`, позиции рисуются сферами (зелёные/красные — стрелок, жёлтые —
  цель), стойку печатает `[LOS] Stance ...`;
- `xru1.AI.LogCombat 1` — решения вражеского AI.

**Что осталось из этого документа:** UI-часть **Ф8** (жёлтый щит), **Ф11**
(превью peek), **Ф12** (крит от фланга, опционально). Ф10 ведётся в аудите 16
A16-P1–A16-P2; остаток **Ф9**
влит в фазу A5 плана AI. Всё это ведётся в
...
**Статус: собрано и принято игроком** («ход переходится»).

### Что сделано — камера кадра выстрела (`EnterShotFraming`)

> ⚠️ **Переписано 2026-07-25 (цикл 17).** Ниже — АКТУАЛЬНАЯ схема. Прежняя
> («подобрать yaw + длину пружины + наклон») и три её дефекта разобраны в
> [13_AI_STATE_MACHINE_PLAN, цикл 17](13_AI_STATE_MACHINE_PLAN.md), находки
> 122–126. Коротко, почему её пришлось менять, а не докручивать: кадр
> **считался в одной точке, а строился в другой** (высота пивота = высота пешки
> камеры, а она не меняется никогда), плечо задавалось УГЛОМ (в упор не
> разводит фигуры вообще), и никто не проверял, видна ли цель.

**Принцип.** Камера считается НАПРЯМУЮ в мире: позиция + точка взгляда.
Параметры пружины выводятся из них (`Rot = (Look − Cam).Rotation()`,
`Arm = |Look − Cam|`), поэтому конец пружины попадает ровно в вычисленную точку
и «посчитано» ≡ «показано».

1. **Базовая позиция «из-за плеча»**: от груди стрелка назад по оси
   стрелок→цель (`ShotFrameBackNear/Far`), вбок **в сантиметрах**
   (`ShotFrameShoulderNear/Far`) и вверх (`ShotFrameHeightNear/Far`); всё три —
   лерп по `Dist/ShotFrameFarDistance`.
2. **`FitSubjectsInFrame`** — отъезд назад по оси взгляда, пока грудь стрелка и
   грудь цели не влезут в `ShotFrameFovSafety` долю FOV (учитывая, что
   вертикальный полуугол уже горизонтального). Композиция стала следствием
   геометрии, а не таблицы зумов.
3. **`PullCameraOutOfGeometry`** — толстый свип точка взгляда → камера, отступ
   `ShotFrameWallPadding`. ⚠️ **Не возвращать `bDoCollisionTest=true`** — это и
   была причина мигания на близких целях.
4. **Лестница кандидатов** — плечо (±) × подъём (0/1/2 шага
   `ShotFrameClearanceLift`), оценка `ScoreShotCandidate`: приоритет «видна
   ЦЕЛЬ» (2.0) → «виден стрелок» (0.7) → «чиста линия до точки взгляда» (0.3),
   минус штраф за высоту подъёма. Так камера приподнимается НАД укрытием вместо
   того, чтобы показывать игроку стену.
5. **Высота пивота — явное состояние `PivotWorldZ`** (мировая координата), из
   неё каждый кадр считается `SpringArm->TargetOffset.Z`. Её ведут focus/follow
   (пол юнита) и кадр выстрела (линия груди). Побочно чинится и тактический
   вид: раньше он смотрел на плоскость высоты `PlayerStart`, а не на плоскость
   бойцов.

**Тюнинг — на `BP_TacticalCameraPawn`** (категория `Tactics|Camera|Shot`):
`ShotFrameAimHeight`, `ShotFrameFarDistance`, `ShotFrameBackNear/Far`,
`ShotFrameShoulderNear/Far`, `ShotFrameHeightNear/Far`,
`ShotFrameTargetBias`/`…Far`, `ShotFrameFovSafety`, `ShotFrameMinArm/MaxArm`,
`ShotFrameClearanceLift`, `ShotFrameWallPadding`, `ShotFrameDuration`.
⚠️ Старые поля (`ShotFrameZoom*`, `ShotFramePitch*`, `ShotFrameYawOffset`,
`ShotFrameLookHeight`) **удалены** — переопределения в BP отпали, действуют
дефолты класса.

### Historical partial fix — автопереход хода (superseded A16-P1)
...
Аудит 2026-07-28 показал, что это закрыло только видимый camera race: fixed
shot-frame не равен завершению montage/StepOut/return, а ability к этому моменту
уже сняла GAS lock. Актуальный контракт — общий `IsActionInProgress(ActionId)`
до terminal completion, см. `16_UNREAL_MCP_TECH_AUDIT.md` A16-P1.
...
3. **Стрельба по врагу В УКРЫТИИ** — проверяется только когда AI научится
   прятаться так, чтобы его можно было пикать. Ведётся в фазе **A5**
   ([13_AI_STATE_MACHINE_PLAN.md](13_AI_STATE_MACHINE_PLAN.md)).
