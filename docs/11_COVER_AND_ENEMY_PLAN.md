...
> ⚠️ **Предусловие, которого сейчас нет в коде:** `EvaluateSurroundings` берёт
> лучший ТИП укрытия и **выбрасывает направление** — прижать бойца к стене
> нечем. Плюс нет единой точки состояния для ABP (состояние размазано по 6
> местам). И то, и другое разобрано и распланировано в
> [13_AI_STATE_MACHINE_PLAN.md](13_AI_STATE_MACHINE_PLAN.md) — фазы **S1**
> (направление укрытия) и **S2** (`FUnitVisualState`). Ф10 начинать после них.
...
**Что осталось из этого документа:** UI-часть **Ф8** (жёлтый щит), **Ф10/Ф11**
(стойки, превью peek), **Ф12** (крит от фланга, опционально). Остаток **Ф9**
