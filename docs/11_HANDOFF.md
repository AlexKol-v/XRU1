...
> Дальнейший план геймплея — в `12_GAMEPLAY_STATUS.md`.
...
**`12_GAMEPLAY_STATUS.md`** (там же 10 блоков доработки: AI, укрытия/peek,
анимации, slow-mo реакций, числа урона, звук, добить HUD).

Вся логика боя — в C++ (`Source/XRU1/Tactics`, `.../UI`). Единый источник
правды режима игрока — `EPlayerTargetingMode` (см. `03_CODE_OVERVIEW §2.6`).
