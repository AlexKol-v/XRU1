...
- [x] ~~Юниты влияют на навмеш (вырезы NavArea_Null)~~ → **переделано
  ...
  *Принятое решение* — **достижимость определяет ТОЛЬКО волновое поле
  ...
  `GetPointAlongPathBudget` **усекает** путь по клиренсу (`GetTransitClearance`
...
>
> Предлагаемый порядок (по возрастанию сложности, каждый пункт — отдельное
> решение/согласование, НЕ пакет):
> - **P0** (дешёво, только UI): баннер/подсказка «ВЫБЕРИТЕ ЦЕЛЬ (ЛКМ) ·
>   ПКМ/Esc — отмена» + другая подсветка наведённого врага, когда
>   `bAwaitingAttackTarget`/`bAwaitingAbilityTarget` = true (нужен
>   `BlueprintPure`-геттер в контроллере + реакция в `UTacticalHUDWidget`).
> - **P1** (небольшая правка контроллера): цикл по видимым врагам
>   Tab/Q/E, пока режим прицеливания вооружён.
> - **P2** (самое трудоёмкое, самый большой визуальный эффект): расширить
>   `ATacticalCameraPawn` состоянием «кадр выстрела» — короткий наезд/поворот
>   на ось стрелок-цель на время выстрела, затем возврат.
> - **P3** (опционально, вопрос темпа игры): разбить подтверждение на 2 клика
>   (клик по врагу → кадр прицеливания; повторный клик/Enter → выстрел) вместо
>   текущего одного клика — медленнее, но более «по-XCOM'овски» церемонно;
>   можно НЕ делать, если важнее скорость боя.
